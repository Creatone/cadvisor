...
func mockResctrlMonData(path string) {
	...
	touchDir(filepath.Join(path, monDataDirName, "mon_L3_00"))
	...
	for _, file := range files {
		touch(file.path)
		ioutil.WriteFile(file.path, []byte(file.value), os.ModePerm)
	}
}
...
func mockContainersPids() string {
	...
	// another
	touchDir(filepath.Join(path, "another"))
	touch(filepath.Join(path, "another", cgroups.CgroupProcesses))
...
func TestGetResctrlPath(t *testing.T) {
	...
	for _, test := range testCases {
		actual, err := getResctrlPath(test.container)
...
func TestGetStats(t *testing.T) {
	...
	for _, test := range testCases {
		containerPath, err := getResctrlPath(test.container)
